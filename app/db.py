...
class MongoDB:
    ...
    async def initialize(self): 
        ...
        logger.info("Database indexes are initialized.")

    async def update_book_outline(self, book_title: str, outline_md: str): 
        """Stores or updates the raw markdown outline for a book."""
        if not book_title or not outline_md:
            logger.warning("Book title or outline is empty. Skipping DB update.")
            return
        ...
        try:
            ...
            if result.upserted_id:
                logger.info(f"Created new book document for '{book_title}' and saved the outline.")
            elif result.modified_count > 0:
                logger.info(f"Updated the outline for book '{book_title}'.")
            return result
        except Exception as e:
            logger.error(f"Failed to update book outline for '{book_title}' in MongoDB: {e}")
            raise
